<p class="payment_module payson_pay_module">
    ...
    {if Configuration::get('PAYSON_INVOICE_ENABLED') == 0 }
    ...
    {/if}
    {if Configuration::get('PAYSON_INVOICE_ENABLED') && Configuration::get('PAYSON_ALL_IN_ONE_ENABLED') == 1 && $paysonInvoiceAmountMinLimit && Context::getContext()->country->iso_code == 'SE'}
        <a href="javascript:disableBtn()"  title="{l s='Payson All in One' mod='paysondirect'} ">
            <img src="{$module_template_dir}paysonAion.png" alt="{l s='Payson All in One' mod='paysondirect'}" />
            <strong>{l s='Payson All in One' mod='paysondirect'}</strong>
        </a>
    {/if}
